    ...
    public void RefreshJobs()
    {
        ...
        foreach (var department in departments)
        {
            ...
            foreach (var job in jobs)
            {
                ...
                if (!protoManager.TryIndex<RoleLoadoutPrototype>(effectiveJobLoadoutId, out var roleLoadoutProto))
                {
                ...
                {
                    var altTitleButton = new OptionButton
                    {
                        Margin = new Thickness(5f, 0f),
                    };

                    altTitleButton.AddItem(job.LocalizedName, 0);

                    for (var i = 0; i < job.AlternativeTitles.Count; i++)
                    {
                        altTitleButton.AddItem(Loc.GetString(job.AlternativeTitles[i]), i + 1);
                    }
                        ...
                        Profile.JobAlternativeTitles.TryGetValue(job.ID, out var savedAltTitle))
                    {
                        ...
                        if (idx >= 0)
                            altTitleButton.SelectId(idx + 1);
                    }

                    altTitleButton.OnItemSelected += args =>
                    {
                        altTitleButton.SelectId(args.Id);
                        Profile = args.Id == 0
                            ? Profile?.WithJobAlternativeTitle(job.ID, null)
                            : Profile?.WithJobAlternativeTitle(job.ID, job.AlternativeTitles[args.Id - 1]);

                        SetDirty();
                    };

                    selector.ReplaceTitleWith(altTitleButton);
                }
