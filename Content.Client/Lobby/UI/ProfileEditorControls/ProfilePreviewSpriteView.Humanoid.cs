...
public sealed partial class ProfilePreviewSpriteView
{
    ...
    private void LoadHumanoidEntity(HumanoidCharacterProfile? humanoid, JobPrototype? job, bool jobClothes)
    {
        ...
        if (humanoid != null && jobClothes)
        {
            ...
            {
                var loadout = humanoid.GetLoadoutOrDefault(jobLoadoutId, _playerManager.LocalSession, humanoid.Species, EntMan, _prototypeManager, sponsorPrototypes);
                GiveDummyLoadout(loadout, jobClothes);
            }
        }
    }
    ...
    private void GiveDummyLoadout(RoleLoadout? roleLoadout, bool outerwear)
    {
        if (roleLoadout == null)
            return;
