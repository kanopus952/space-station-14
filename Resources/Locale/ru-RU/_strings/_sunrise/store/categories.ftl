# donate plinks
store-category-general = Общее
store-category-music = Музыка
store-category-cloaks = Плащи
store-category-friends = Друзья
store-category-special = Плюшки
store-category-hos = ГСБ
store-category-ce = Старший инженер
store-category-cmo = Главный врач
store-category-rd = Научный руководитель
store-category-qm = Квартирмейстер
store-category-hop = Глава персонала
store-category-iaa = АВД
store-category-captain = Капитан
store-category-service = Сервисный отдел
store-category-medical = Медицинский отдел
store-category-science = Научный отдел
store-category-engineering = Инженерный отдел
store-category-cargo = Отдел снабжения
store-category-security = Служба Безопасности
store-category-paint = Краска
# disease
shop-disease-category-infect = Передача
shop-disease-category-symptoms = Симптомы
shop-disease-category-evolution = Улучшение
# Uplink
store-category-objectives = Цели
store-category-mechs = Мехи
store-category-cybernetics = Кибернетика
# ntUplink
store-category-ntautogun = Автоматы
store-category-ntlasers = Лазерное оружие
store-category-ntmedicine = Медицина
store-category-ntpistols = Пистолеты
store-category-ntadditional = Дополнительное
store-category-ntsanitary = Санитария
store-category-ntengineering = Инженерия
store-category-ntmechs = Мехи
store-category-ntexplosive = Взрывчатка
store-category-ntammo = Боеприпасы
store-category-ntequipment = Экипировка
# PirateUplink
store-category-exchanger = Обмен
