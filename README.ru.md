...
Pull Request с C# ограничены.

Вы можете отправлять C# PR только если:

* Вы состоите в команде разработки
  ИЛИ
* Ваш PR привязан к Issue с меткой `Одобрено`

Неодобренные идеи, случайные рефакторы и незапрошенные системы могут быть закрыты без ревью.
...
* Проходить все тесты и CI
* Проходить AI Review
* Иметь закрытыми все комментарии AI Review перед merge

PR с проваленными тестами или незакрытыми комментариями не будут замержены.

## GDD и соответствие проекту

Любой контент должен соответствовать GDD (Game Design Documentation).

Для общих репозиториев, таких как SUNRISE, контент должен подходить всем серверным диздокам одновременно.
