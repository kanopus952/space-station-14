...
public abstract partial class SharedImplanterSystem : EntitySystem
{
    ...
    private void OnEntInserted(Entity<ImplanterComponent> ent, ref EntInsertedIntoContainerMessage args)
    {
        ...
        ChangeOnImplantVisualizer(ent); // Sunrise-Edit
        Dirty(ent);
    }

    // Sunrise added start - очистка данных и обновление вида после извлечения импланта
    private void OnEntEjected(Entity<ImplanterComponent> ent, ref EntRemovedFromContainerMessage args)
    {
        if (_timing.ApplyingState)
            return;
        ...
        ent.Comp.ImplantData = (string.Empty, string.Empty);
    ...
    {
        // Sunrise added start - ограничение одноразовых имплантеров на применение к себе
        if (ent.Comp.OnlySelfImplant && target != user)
        {
            implant = null;
            implantComp = null;
            var selfMessage = Loc.GetString("implanter-only-self-implant");
            _popup.PopupEntity(selfMessage, ent, user);
            return false;
        }
        // Sunrise added end
