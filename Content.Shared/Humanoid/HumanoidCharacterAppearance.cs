...
public sealed partial class HumanoidCharacterAppearance : IEquatable<HumanoidCharacterAppearance>
{
    ...
    {
        // Sunrise edit start - полная рандомизация сохраняет волосы, выборочная не стирает маркировки
        var fullRandom = (charEditorRandomizeConfig & (RandomizeConfigAll ^ RandomizeCfg.Species)) == (RandomizeConfigAll ^ RandomizeCfg.Species);
        var appearance = new HumanoidCharacterAppearance(baseAppearance);
        if (fullRandom)
            appearance.Markings = CreateRandomHairMarkings(species, sex, IoCManager.Resolve<IRobustRandom>(), IoCManager.Resolve<MarkingManager>());
        // Sunrise edit end
