    ...
    public sealed partial class HumanoidCharacterProfile
    {
        ...
        public static SpeciesPrototype RandomSpecies(HashSet<string>? ignoredSpecies = null)
        {
            ...
            var pool = prototypeManager.EnumeratePrototypes<SpeciesPrototype>()
                .Where(x => x.RoundStart && !x.SponsorOnly && (ignoredSpecies == null || !ignoredSpecies.Contains(x.ID))) // Sunrise-Edit
                .ToArray();
        ...
        public static HumanoidCharacterProfile Random(HashSet<string>? ignoredSpecies = null)
        {
            ...
            if (ignoredSpecies != null)
            {
                ...
                config &= ~RandomizeCfg.Species; // Sunrise-Edit - не выбираем повторно исключённую расу
            }
        ...
        public static HumanoidCharacterProfile Random(RandomizeCfg randomizeCfg, HumanoidCharacterProfile baseProfile)
        {
            ...
            var profile = new HumanoidCharacterProfile(baseProfile); // Sunrise-Edit - не теряем настройки при выборочной рандомизации
            ...
            if (!speciesProto.Sexes.Contains(profile.Sex)) // Sunrise-Edit - пол должен подходить новой расе
                profile.Sex = speciesProto.Sexes[0];
            profile.Voice = profile.Species == baseProfile.Species && profile.Sex == baseProfile.Sex && speciesProto.Voices.Contains(baseProfile.Voice)
                ? baseProfile.Voice
                : speciesProto.DefaultSoundsBySex[(int)profile.Sex]; // Sunrise-Edit - выборочная рандомизация не меняет голос
            ...
            if (fullRandom)
                profile.SunriseProfile = SunriseCharacterProfile.RandomForSpecies(speciesProto, profile.Sex, IoCManager.Resolve<IRobustRandom>(), prototypeManager);
            else if (profile.Species != baseProfile.Species || profile.Sex != baseProfile.Sex)
                profile.AdaptSunriseRandomProfile(speciesProto, prototypeManager);
            // Sunrise edit end
