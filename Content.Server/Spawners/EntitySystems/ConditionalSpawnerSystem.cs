...
public sealed partial class ConditionalSpawnerSystem : EntitySystem
{
    ...
    private void Spawn(EntityUid uid, ConditionalSpawnerComponent component)
    {
        ...
        SpawnWithStorytellerPropagation(uid, toSpawn, coords, rotation); // Sunrise-Edit - переносим служебный тег на результат.
    }
    ...
    private void Spawn(EntityUid uid, RandomSpawnerComponent component)
    {
        ...
        SpawnWithStorytellerPropagation(uid, proto, coordinates, rotation); // Sunrise-Edit - учитываем выбранный редкий прототип и служебный тег.
    }
    ...
    private void Spawn(Entity<EntityTableSpawnerComponent> ent)
    {
        ...
        {
            SpawnAtRandomOffset(spawns, ent, coords, comp.Offset, rotation); // Sunrise-Edit - переносим служебный тег на результаты.
        }
    }
        ...
        Angle rotation)
    {
        ...
        SpawnAtRandomOffset(nonStackable, ent, coords, offset, rotation); // Sunrise-Edit - переносим служебный тег на результаты.
        ...
        foreach (var (protoId, count) in prototypeStacks.Values)
        {
            ...
            SpawnStacksWithStorytellerPropagation(ent, protoId, count, entCoordinates); // Sunrise-Edit - переносим тег на каждую стопку.
        }
    }

    private void SpawnAtRandomOffset(IEnumerable<EntProtoId> spawns,
        EntityUid spawner,
        MapCoordinates coords,
        float offset,
        Angle rotation)
    {
        ...
        {
            SpawnAtRandomOffset(proto, spawner, coords, offset, rotation);
        }
    }

    private EntityUid SpawnAtRandomOffset(EntProtoId proto,
        EntityUid spawner,
        MapCoordinates coords,
        float offset,
        Angle rotation)
    {
        ...
        return SpawnWithStorytellerPropagation(spawner, proto, trueCoords, rotation); // Sunrise-Edit
    }
