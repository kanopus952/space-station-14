...
public sealed partial class DisposalHolderSystem : SharedDisposalHolderSystem
{
    ...
    public override void Exit(Entity<DisposalHolderComponent> ent)
    {
        ...
        EntityUid? sunriseAutoLoader = null; // Sunrise-Edit
        ...
        if (TryComp<MapGridComponent>(gridUid, out var grid))
        {
            foreach (var contentUid in _maps.GetLocal(gridUid.Value, grid, xform.Coordinates))
            {
                ...
                // Sunrise added start - выход держателя в автозагрузчик шаттельной пушки
                if (IsSunriseAutoLoader(contentUid))
                {
                    sunriseAutoLoader = contentUid;
                    break;
                }
                // Sunrise added end
            }
            ...
            if (unit == null && sunriseAutoLoader == null && _net.IsServer) // Sunrise-Edit
            {
        ...
        while (children.MoveNext(out var held))
        {
            ...
            {
                // Sunrise added start - передача содержимого держателя в автозагрузчик
                if (sunriseAutoLoader is { } autoLoader &&
                    ent.Comp.CurrentTube is { } currentTube &&
                    TryCycleSunriseAutoLoader(held, autoLoader, currentTube))
                {
                    continue;
                }
                // Sunrise added end
