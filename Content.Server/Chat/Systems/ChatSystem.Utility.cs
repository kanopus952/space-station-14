...
public sealed partial class ChatSystem
{
    ...
    /// </summary>
    public void SendInVoiceRange(ChatChannel channel, string message, string wrappedMessage, EntityUid source, ChatTransmitRange range, NetUserId? author = null, Color? color = null) // Sunrise-Edit
    {
        foreach (var (session, data) in GetRecipients(source, VoiceRange))
        {
            ...
            // Sunrise added start - эмоции видны только при прямой видимости
            if (channel == ChatChannel.Emotes)
            {
                var visibility = new EmoteVisibilityCheckEvent(source, session.AttachedEntity, VoiceRange);
                RaiseLocalEvent(ref visibility);
            ...
            _chatManager.ChatMessageToOne(channel, message, wrappedMessage, source, entHideChat, session.Channel, author: author, colorOverride: color); // Sunrise-Edit
        }
    ...
    public string SanitizeMessageReplaceWords(string message)
    {
        ...
        msg = _wordreplacement.ApplyReplacements(msg, ChatSanitizeAccent); // Sunrise-Edit
