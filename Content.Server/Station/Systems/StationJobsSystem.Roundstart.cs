...
public sealed partial class StationJobsSystem
{
        ...
        out NetUserId player)
    {
        ...
        for (var priority = JobPriority.High; priority > JobPriority.Never; priority--)
        {
            ...
            // Sunrise-Edit - сохраняем приоритет спонсоров при распределении ролей.
            player = _sponsorsManager?.PickRoleSession(players, job.Id) ?? _random.Pick(players);
            return true;
        }
        ...
        IReadOnlyDictionary<NetUserId, HumanoidCharacterProfile> profiles)
    {
        ...
        foreach (var (player, profile) in profiles)
        {
            ...
            foreach (var jobId in profileJobs)
            {
                ...
                // Sunrise-Edit - учитываем ограничения роли по расе персонажа.
