        ...
        for (var i = 0; i < words.Length; i++)
        {
            ...
            if (_random.NextDouble() >= 0.5)
            {
                if (word.Length > 1)
                {
                    ...
                    {
                        accentedMessage.Append('У');  // Russian-Localization
                    }

                    if (_random.NextDouble() >= 0.3)
                        accentedMessage.Append('К');  // Russian-Localization
                }
                else
                    accentedMessage.Append('У');  // Russian-Localization
            }
            else
            {
                foreach (var __ in word)
                {
                    if (_random.NextDouble() >= 0.8)
                        accentedMessage.Append('Г');  // Russian-Localization
                    else
                        accentedMessage.Append('А');  // Russian-Localization
                }
